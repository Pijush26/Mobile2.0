...
public class ExtentTestCaseHandler extends TestCaseHandler implements PrimaryHandler {
		...
		if (isExtentEnabled()) {
			try {
				...
				browserName = runContext.BrowserName;
                                browserVersion = runContext.BrowserVersion;
	...
	public void updateTestLog(String stepName, String stepDescription, Status state, String link, List<String> links) {
		...
		try {
			...
			if (isExtentEnabled()) {
				try {
					sendLog(state.toString(), stepData, filename);
	...
	public void startComponent(String component, String desc) {
		...
		isIteration = false;
		this.originalState = this.test;
		this.test = this.test.createNode(component);
	}
	...
	public void sendLog(String status, String teststepdata, String filename) throws IOException, ParseException {
            		if (filename.equalsIgnoreCase("")) {
			if (status.contains("DONE")||status.contains("COMPLETE")) {
				this.test.info(teststepdata.replace("[" + status + "]", "").replace("|", "").trim());
			} else if (status.contains("PASS")) {
				this.test.pass(MarkupHelper.createLabel(
						teststepdata.replace("[" + status + "]", "").replace("|", "").trim(), ExtentColor.GREEN));
			} else {
				this.test.fail(MarkupHelper.createLabel(
						teststepdata.replace("[" + status + "]", "").replace("|", "").trim(), ExtentColor.RED));
			}
		} else {
			...
			if (status.contains("DONE")||status.contains("COMPLETE")) {
				...
				this.test.pass(teststepdata.replace("[" + status + "]", "").replace("|", "").trim(),MediaEntityBuilder.createScreenCaptureFromPath(filename).build());
			} else {
				this.test.fail(teststepdata.replace("[" + status + "]", "").replace("|", "").trim(),MediaEntityBuilder.createScreenCaptureFromPath(filename).build());
			}
		}
	}
}
