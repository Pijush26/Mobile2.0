...
public class AzureTestCaseHandler extends TestCaseHandler implements PrimaryHandler {
	...
	public void createLogNodes(String stepdata, String status, String filename) throws IOException, ParseException {
		...
		if (filename.equalsIgnoreCase("")) {
			...
			filename = screenshot.getAbsolutePath();
			if (status.contains("PASS") || status.contains("DONE") || status.contains("COMPLETE")) {

			} else {
				this.stacktraceData += "Step " + getStepCount() + ":   " + stepdata + "\n";
			}
			attachments += "<attachment><filePath>" + filename + "</filePath></attachment>" + "\n";
		}
